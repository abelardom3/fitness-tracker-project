...
app.get('/api/logs', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT * FROM fitness_tracker')
        res.send(rows)
    } catch (error) {
...
app.put('/api/logs/:id', async (req, res) => {
    ...
    try {
        await pool.query('UPDATE fitness_tracker SET workout=$1,duration=$2, date=$3 WHERE fit_id =$4 ', [workout, duration, date, id])
